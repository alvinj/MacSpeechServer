...
/**
 * TODO this class needs some work. it was originally created for my "Wikipedia Reader" project,
 * and the code was written (quickly) for that, and needs to be refactored, or thrown out if it's
 * not needed for this project. (Caveat: Some characters cause a problem for the AppleScript `say`
 * command, so you do need to massage the text a bit to keep that problem from happening.)
 */
object TextUtils {
